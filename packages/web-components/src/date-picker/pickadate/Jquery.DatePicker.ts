...
export class DatePicker {
    ...
    constructor(container: any, options: any) {
        ...
        let $dateField: any = ($datePicker.find('.ms-TextField-field') as any).pickadate(
            $.extend(
                {
        ...
        $picker.on({
            render: () => {
                this.updateCustomView($datePicker as any);
            }
        });
    }
     ...
     */
    public initCustomView($datePicker: any) {
        ...
        /** Move back one month. */
        $monthControls.on('click', '.js-prevMonth', (event: any) => {
        ...
        $monthControls.on('click', '.js-nextMonth', (event: any) => {
            event.preventDefault();
            let newMonth = $picker.get('highlight').month + 1;
            this.changeHighlightedDate([null, newMonth, null]);
        });
        ...
        /** Move back one year. */
        $monthPicker.on('click', '.js-prevYear', (event: { preventDefault: () => void }) => {
            event.preventDefault();
        ...
        /** Move ahead one year. */
        $monthPicker.on('click', '.js-nextYear', (event: { preventDefault: () => void }) => {
            event.preventDefault();
            let newYear = $picker.get('highlight').year + 1;
            this.changeHighlightedDate([newYear, null, null]);
        });
        ...
        $yearPicker.on('click', '.js-prevDecade', (event: { preventDefault: () => void }) => {
            event.preventDefault();
        ...
        $yearPicker.on('click', '.js-nextDecade', (event: { preventDefault: () => void }) => {
            event.preventDefault();
            let newYear = $picker.get('highlight').year + 10;
            this.changeHighlightedDate([newYear, null, null]);
        });
        ...
        $goToday.click((event: { preventDefault: () => void }) => {
            event.preventDefault();
        ...
        $monthPicker.on('click', '.js-changeDate', (event: { preventDefault: () => void; target: any }) => {
            event.preventDefault();
        ...
        $yearPicker.on('click', '.js-changeDate', (event: { preventDefault: () => void; target: any }) => {
            event.preventDefault();
            let $changeDate = $(event.target);
    ...
    public changeHighlightedDate(dateArr: any[]) {
    ...
    public updateCustomView($datePicker: {
        find: (arg0: string) => { (): any; new (): any; pickadate: { (arg0: string): any; new (): any } };
    }) {
        /** Get some letiables ready. */
        let $monthPicker: any = $datePicker.find('.ms-DatePicker-monthPicker');
        let $yearPicker: any = $datePicker.find('.ms-DatePicker-yearPicker');
        let $picker: any = $datePicker.find('.ms-TextField-field').pickadate('picker');
